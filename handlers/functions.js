  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322);
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, pre+".showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).length}]__**`, role.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, pre+".rosteremoji")} ***\`${role.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).length - 20}\` other Members have this Role ...***`: ""}`.substring(0, 1024), the_roster_db?.get(guild.id, pre+".inline"))
              break;
            } catch (e) {
              console.error(e)
            }
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, pre+".inline"))
            } catch (e) {
              console.error(e)
            }
        }
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for(const reaction of msg.reactions.cache.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322)){                      
 ...
 */
  ...
  let button_back = new MessageButton().setStyle('PRIMARY').setCustomId('1').setEmoji("975381256193843240").setLabel("Back")
  let button_home = new MessageButton().setStyle('SECONDARY').setCustomId('2').setEmoji("🏠").setLabel("Home")
  let button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('3').setEmoji('975381288267677716').setLabel("Forward")
  let button_blank = new MessageButton().setStyle('SECONDARY').setCustomId('button_blank').setLabel("\u200b").setDisabled();
  let button_stop = new MessageButton().setStyle('SECONDARY').setCustomId('stop').setEmoji("🛑").setLabel("Stop")
...
async function swap_pages2_interaction(client, interaction, embeds) {
  ...
  let button_back = new MessageButton().setStyle('PRIMARY').setCustomId('1').setEmoji("937211085956730930").setLabel("Back")
  let button_home = new MessageButton().setStyle('SECONDARY').setCustomId('2').setEmoji("🏠").setLabel("Home")
  let button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('3').setEmoji('937211016293523517').setLabel("Forward")
  let button_blank = new MessageButton().setStyle('SECONDARY').setCustomId('button_blank').setLabel("\u200b").setDisabled();
  let button_stop = new MessageButton().setStyle('SECONDARY').setCustomId('stop').setEmoji("🛑").setLabel("Stop")
  const allbuttons = [new MessageActionRow().addComponents([button_back, button_home, button_forward, button_blank, button_stop])]
  ...
  for (let i = 0; i < guilds.length; i++) {
      try {
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE" && jointocreate.includes(ch.id)).each(async (channel, j) => {
              try{
                  let members = channel.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322);
            ...
            guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE").each(async vc => {
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      let members = vc.members.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322);
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(this_Code_is_by_Ron_7322 => this_Code_is_by_Ron_7322).reverse(); //reverse the array to have it listed like the discord chat
